...
class plgVmPaymentwirecardceecheckout extends vmPSPlugin
{
	...
	protected static $PLUGIN_VERSION = '1.6.0';
	...
	public function plgVmOnSelectCheckPayment(VirtueMartCart $cart, &$msg)
	{
		...
		$birthDay = $input->get('wcp_day');
		$birthMonth = $input->get('wcp_month');
		$birthYear = $input->get('wcp_year');
		$consentInvoice = 'off';
		...
		$sessionWirecard->birthDay = $birthDay;
	...
	protected function _initiatePayment($cart)
	{
		try {
			...
			if ($this->_sendShippingInformation()
			    || ($paymentType == WirecardCEE_QPay_PaymentType::INVOICE && $this->_getMethod()->invoice_provider != 'payolution')
			    || ($paymentType == WirecardCEE_QPay_PaymentType::INSTALLMENT && $this->_getMethod()->installment_provider != 'payolution')
			) {
